...
describe('files.service.spec.ts', () => {
    ...
    const legalCaseId = 'f423a921-31d7-44d5-aa01-f8440c88d96e';
    ...
    it('Test 2. Subir un video que si necesita dividirse',
        async () => {
        ...
        }, 600000 // 10 min de timeout
    );

    it('Test 3. Subir un pdf',
        async () => {
    ...
    it('Test 4. Subir un video de un segundo que no puede generar un cache',
        async () => {
            const originalname = 'videoOneSecond.mp4';
            const fileForTest = {
                originalname,
                mimetype: 'video/mp4',
                size: 169161,
            ...
            //copiamos el archivo a la carpeta temporal simulando a multer
            const sourcePath = `./static/videos/${originalname}`;
            const destinationPath = `./filesUploadedTemp/${originalname}`
            try {
                copyFileSync(sourcePath, destinationPath);
            } catch (error) {
                fail(`Error al copiar archivo -> ${error}`);
            ...
            //subimos el video
            console.log(`Subiendo un video que no necesita dividirse -> ${originalname}`);
            await filesService.upload(
                fileForTest.originalname,
                fileForTest.mimetype,
                fileForTest.size,
    ...
    it('Test de comprobación Vemos todos los archivos del usuario ṕor segunda vez',
        async () => {
            try {
